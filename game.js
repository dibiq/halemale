  ...
  ensureAvatarAnimation(baseKey) {
    ...
    try {
      ...
      if (baseKey === "player_1" || baseKey === "player_2" || baseKey === "player_3" || baseKey === "player_4" || baseKey === "player_5") {
        ...
        // Ensure frames are loaded/generated for player_5 and player_6
        if (baseKey === "player_5") {
          ensurePlayer5Frames(this);
        } else if (baseKey === "player_6") {
          ensurePlayer6Frames(this);
        }
  ...
  showQuestPopup() {
    ...
    if (typeof this.updateIapButtonState === "function") {
      try {
        ...
        console.warn("[LobbyScene.create] updateIapButtonState failed", e);
      }
    }
          ...
          if (isVideo) {
            ...
            const canUseAd = () => {
              try {
                ...
                console.warn("[daily-ad] 광고 지원 확인 실패", error);
                return false;
              }
            };
            ...
            if (!adGroupId || !canUseAd()) {
              ...
              console.warn("[daily-ad] 광고 지원 불가", {
                hasAdGroupId: !!adGroupId,
                canUse: canUseAd(),
              });
              return;
            }
            ...
            const applyDailyAdReward = () => {
              try {
                ...
                if (!scene) {
                  console.error('[daily-ad] ❌ scene이 없음');
                  return;
                }
                ...
                if (typeof socket === "undefined") {
                  console.error('[daily-ad] ❌ socket이 없음');
                ...
                try {
                  ...
                  console.log('[daily-ad] ✅ showCoinBurstEffect 호출');
                } catch (e) {
                  console.warn('[daily-ad] ⚠️ showCoinBurstEffect 실패:', e.message);
                }
                ...
                try {
                  if (rowDateStr) {
                    ...
                    try {
                      ...
                      console.log('[daily-ad] ✅ markDailyRewardClaimed 호출');
                    } catch (e) {
                      console.warn('[daily-ad] ⚠️ markDailyRewardClaimed 실패:', e.message);
                    }
                    ...
                    console.log('[daily-ad] ✅ 스탠프 생성 및 애니메이션 호출');
                  }
                } catch (e) {
                  console.warn('[daily-ad] ⚠️ 스탠프 생성 실패:', e.message);
                }

                // 3. 토스트 표시
                if (typeof scene.showToast === 'function') {
                  console.log('[daily-ad] ✅ 토스트 표시');
                ...
                console.error('[daily-ad] 스택:', e.stack);
                
                // 에러 발생해도 반드시 상태 초기화
                console.log('[daily-ad] 에러 발생 - 상태 초기화 시작');
                try {
                  ...
                  console.log('[daily-ad] ✅ 에러 후 상태 초기화 완료');
                } catch (cleanupError) {
                  console.error('[daily-ad] ❌ 상태 초기화 중 에러:', cleanupError.message);
                }
              }
            };
            ...
            if (isGameAdShowing) {
              console.log("[daily-ad] 이미 광고 재생 중");
            ...
            if (isGameAdLoaded) {
              ...
              scene.unregisterShowAd = showFullScreenAd({
                options: { adGroupId },
                onEvent: (event) => {
                  console.log("[daily-ad] showFullScreenAd 이벤트 상세:", {
                    type: event.type,
                  ...
                  console.log("[daily-ad] 정규화된 이벤트 타입:", eventType);
      ...
      try {
        ...
        if (baseTexture && this.textures.exists(baseTexture)) {
          ...
          try {
            ...
            console.log("[playWinAnimation] before applyAvatarAnimation for:", avatarKey);
            
            // Ensure player_5 and player_6 frames are loaded
            if (avatarKey === "player_5") {
              ensurePlayer5Frames(this);
              console.log("[playWinAnimation] ensured player_5 frames");
            } else if (avatarKey === "player_6") {
              ensurePlayer6Frames(this);
              console.log("[playWinAnimation] ensured player_6 frames");
            }
            
            this.applyAvatarAnimation(tempSprite, avatarKey);
            console.log("[playWinAnimation] after applyAvatarAnimation, anims:", tempSprite.anims ? { isPlaying: tempSprite.anims.isPlaying, currentAnim: tempSprite.anims.currentAnim?.key } : "none");
            try {
