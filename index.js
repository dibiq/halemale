...
function applyCoinCardReward(room, player, io) {
  ...
  if (targetSocket) {
    ...
    io.to(targetSocket.id).emit("myProfile", {
      ...
      bellCorrect: Number(targetSocket.bellCorrect) || 0,
      bellTotal: Number(targetSocket.bellTotal) || 0,
...
async function finalizeGame(room, io, { winner, sorted, message }) {
  ...
  io.to(room.roomId).emit("gameEnded", {
    ...
    ...(room.players.some(
      (p) =>
        p?.current_character === "player_12" || p?.avatarKey === "player_12",
    ) && {
      _player12Debug: room.players
        .filter(
          (p) =>
            p?.current_character === "player_12" ||
            p?.avatarKey === "player_12",
        )
        .map((p) => ({
          nickname: p.nickname,
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", async (data) => {
    ...
    if (!joinedPlayer) {
      ...
      // 🔴 [디버그] player_12 저장 확인
      if (
        playerData.avatarKey === "player_12" ||
        playerData.current_character === "player_12"
      ) {
  ...
  socket.on("startGameRequest", async (data, ack) => {
    ...
    io.to(room.roomId).emit("gameStart", {
      ...
      gameMultiplier: room.gameMultiplier || 1, // 🔴 【중요】게임 시작 시 배수 동기화
    });
